...
$(document).ready(function() 
{
	...
	if(divRequest == 'add')  
	{
		// add order	
		// top nav child bar 
		$('#topNavAddpurchaseOrder').addClass('active');	
		...
		$("#createOrderForm").unbind('submit').bind('submit', function()
		{
			...
			{       			
				...
				{	    		    	
					$("#"+productNameId+"").after('<p class="text-danger"> Row-material Name Field is required!! </p>');
					...
					$.ajax({
						...
						success:function(response) 
						{
							...
							{								
								...
								' <br /> <br /> <a type="button" onclick="printOrder('+response.pOrderID+')" class="btn btn-primary"> <i class="glyphicon glyphicon-print"></i> Print </a>'+
								'<a href="purchaseorder.php?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Add New Order </a>'+							
								'</div>');
	...
	{
		// top nav child bar 
		$('#topNavManagepurchaseOrder').addClass('active');

		manageOrderTable = $("#manageOrderTable").DataTable({
			'ajax'	: 'php_action/fetchPurchaseOrder.php',
			'order'	: []
		});		
				...
				if(validateProduct == true && validateQuantity == true) 
				{
						...
						success	:function(response) 
						{
							...
							$('.form-group').removeClass('has-error').removeClass('has-success');
								...
								'<button type="button" class="close" data-dismiss="alert">&times;</button>'+
								'<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> '+ response.messages +
								' <br /> <br /> <a type="button" onclick="printGRN('+response.grnID+')" class="btn btn-primary"> <i class="glyphicon glyphicon-print"></i> Print </a>'+
								'<a href="purchaseorder.php?o=manord" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Manage Order </a>'+							
								'</div>');
...
function printGRN(grnID = null) 
{
	if(grnID) 
	{			
		$.ajax({
			url		: 'php_action/printGRN.php',
			type	: 'post',
			data	: {grnID: grnID},
			dataType: 'text',
			success	:function(response) 
...
/*function paymentOrder(orderId = null) {
	if(orderId) {
		...
		$.ajax({
			url: 'php_action/fetchPurchaseOrderData.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'json',
			success:function(response) {				
