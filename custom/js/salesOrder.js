...
$(document).ready(function() 
{
	...
	if(divRequest == 'add')  
	{
		// add order	
		// top nav child bar 
		$('#topNavAddsalesOrder').addClass('active');	
		
		$("#CustomerName").on('change',function()
		{
			$("#CustomerContact").val($("#CustomerName option:selected").attr("name"));
		});
		...
		{       						
			...
			if(CustomerName &&  discount && paymentType ) 
			{
					...
					$.ajax({
						...
						success:function(response) 
						{
							...
							{								
								...
								' <br /> <br /> <a type="button" onclick="printSalesOrder('+response.sOrderID+')" class="btn btn-primary"> <i class="glyphicon glyphicon-print"></i> Print </a>'+
								'<a href="salesOrders.php?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Add New Order </a>'+
								'</div>');
	...
	{
		// top nav child bar 
		$('#topNavManageselsOrder').addClass('active');
		manageOrderTable = $("#manageOrderTable").DataTable({
			'ajax'	: 'php_action/fetchSalesOrder.php',
			'order'	: []
		});		
	...
	else if(divRequest == 'editOrd') 
	{
		...
		$("#editOrderForm").unbind('submit').bind('submit', function() {
			...
			if(CustomerName == "") 
			{
				$("#CustomerName").after('<p class="text-danger"> The Client Name field is required </p>');
				$('#CustomerName').closest('.form-group').addClass('has-error');
			} 
			else 
			{
			...
			if(CustomerContact == "") {
				$("#CustomerContact").after('<p class="text-danger"> The Contact field is required </p>');
		...
		{       						
			...
			if(orderDate && CustomerName && CustomerContact && paid && discount && paymentType && paymentStatus) 
			{
				if(validateProduct == true && validateQuantity == true) 
				{
					// create order button
...
function printSalesOrder(orderId = null) 
{
	if(orderId) 
	{			
		$.ajax({
			url		: 'php_action/printSalesOrder.php',
			type	: 'post',
		...
		else 
		{
			$.ajax({
				...
				success:function(response) {
					...
					$("#rateValue"+row).val(response.prouctRate);
...
function removeOrder(orderId = null) 
{
	if(orderId) 
	{
		$("#removeOrderBtn").unbind('click').bind('click', function() 
		{
			...
			$.ajax({
				url: 'php_action/removeSalesOrder.php',
				type: 'post',
				data: {orderId : orderId},
				dataType: 'json',
				success:function(response) 
...
function paymentOrder(orderId = null) 
{
	if(orderId) 
	{
		//$("#orderDate").datepicker();
		$.ajax({
			url: 'php_action/fetchsalesOrderData.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'json',
			success:function(response) 
